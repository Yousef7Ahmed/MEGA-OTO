...
router.post("/megaai/order", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[mega-webhook] Body was not valid JSON:",
      String(req.body).slice(0, 300),
    );
    return res
      .status(400)
      .json({ success: false, error: "Payload was not valid JSON" });
  }

  console.log(
    "[mega-webhook] Webhook ping received for order id:",
    webhookPayload.id,
    "- status:",
    webhookPayload.order_status_text,
  );

  const orderId = String(webhookPayload.id ?? webhookPayload.order_id ?? "");
  if (!orderId) {
    console.error(
      "[mega-webhook] Webhook payload has no order id - cannot proceed.",
    );
    return res
      .status(422)
      .json({ success: false, error: "No order id in webhook payload" });
  }
  ...
  if (existing && existing.status === "sent_to_oto") {
    console.log(
      `[mega-webhook] Order ${orderId} was already sent to OTO earlier - skipping re-creation.`,
    );
  ...
  try {
    ...
    console.log(
      "[mega-webhook] Fetched full order details:",
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to process order", details });
  }
});
